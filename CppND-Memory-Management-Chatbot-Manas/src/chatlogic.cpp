...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
    ...
    ChatBot local_chatbot = ChatBot("../images/chatbot.png"); //instantiating a local chatbot on the stack

    local_chatbot.SetChatLogicHandle(this);     //setting the chat logic handle to this class

    local_chatbot.SetRootNode(rootNode);    // the root node will be the rootnode which we just computed

    _chatBot = &local_chatbot;      //the chatbot instance is the local chatbot which we just instantiated

    rootNode->MoveChatbotHere(std::move(local_chatbot));    //passing the chatbot instance into the root node
